...
%token T_EndOfFile T_Return T_Number T_True T_False T_ID T_Print T_Cln T_NL T_EQL T_NEQ T_EQ T_GT T_LT T_EGT T_ELT T_Or T_And T_Not T_In ID ND DD T_String T_If T_Elif T_While T_Else T_Import T_Break T_Pass T_MN T_PL T_DV T_ML T_OP T_CP T_OB T_CB T_Def T_Comma T_List T_For T_Range
...
%type<node> StartDebugger args start_suite suite end_suite func_call call_args StartParse finalStatements arith_exp bool_exp term constant basic_stmt cmpd_stmt func_def list_index import_stmt pass_stmt break_stmt print_stmt if_stmt elif_stmts else_stmt while_stmt for_stmt return_stmt assign_stmt bool_term bool_factor iterable
...
for_stmt : T_For T_ID T_In iterable T_Cln start_suite {$$ = createOp("For", 2, $4, $6);};

iterable : T_Range T_OP constant T_Comma constant T_CP {$$ = createOp("Range",2,$3 ,$5);};
